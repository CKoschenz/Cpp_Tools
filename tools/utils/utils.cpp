...
template std::string prd<int>(int const x, const int decDigits,
                              const int width);
template std::string prd<unsigned int>(unsigned int const x,
                                       const int decDigits, const int width);
template std::string prd<size_t>(size_t const x, const int decDigits,
                                 const int width);
template std::string prd<double>(double const x, const int decDigits,
                                 const int width);
template std::string prd<std::complex<double>>(std::complex<double> const x,
                                               const int decDigits,
                                               const int width);
...
Modifier::Modifier(size_t pCode_FG)
    : code_FG(pCode_FG), code_BG(49), code_FONT(0) {}
...
std::ostream &operator<<(std::ostream &os, const Modifier &mod) {
  return os << "\033[" << mod.code_FONT << ";" << mod.code_BG << ";"
            << mod.code_FG << "m";
}
...
T string_to_T(std::string const &val) {
  ...
  T returnVal;
  if (!(istr >> returnVal))
    ExitWithError("CFG: Not a valid " + (std::string) typeid(T).name() +
                  " received!\n");
  return returnVal;
}
...
bool string_to_T(std::string const &val) {
  ...
  if (!(istr >> std::boolalpha >> returnVal))
    ExitWithError("CFG: Not a valid " + (std::string) typeid(bool).name() +
                  " received!\n");
  return returnVal;
}
...
void show_numerical_limits() {
  ...
  std::cout << "Max Size_t: " << std::numeric_limits<size_t>::max()
            << std::endl;
  std::cout << "Min Size_t: " << std::numeric_limits<size_t>::min()
            << std::endl;
